...
class CrawlListPageObserver extends CrawlObserver {
...
//        echo "Now crawling: " . (string) $url . PHP_EOL;
    ...
    ): void
    {
        ...
        $this->CustomCrawler($url, new CrawlListArticleObserver());
        ...
        if (!empty($linkNext)) {
            ...
            $this->CustomCrawler($linkPage, new CrawlListArticleObserver()); // danh sach bai viet trong 1 page

            $this->CustomCrawler($linkPage, new CrawlListPageObserver()); // lay link page tiep theo
        }
    }

    public function CustomCrawler($link, $Observe){
        Cr::create([RequestOptions::ALLOW_REDIRECTS => true, RequestOptions::TIMEOUT => 30])
            ->acceptNofollowLinks()
            ->ignoreRobots()
            ->setCurrentCrawlLimit(1)
//            ->setParseableMimeTypes(['text/html', 'text/plain'])
            ->setCrawlObserver($Observe)
